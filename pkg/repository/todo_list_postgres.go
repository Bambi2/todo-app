...
func (r *TodoListPostgres) CreateList(userId int, todoList domain.TodoList) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
	...
	row := r.db.QueryRow(createListQuery, todoList.Title, todoList.Description)
...
func (r *TodoListPostgres) UpdateList(userId int, listId int, input domain.UpdateListInput) error {
